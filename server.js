...
io.on('connection', (socket) => {
    ...
    socket.on('set username', (username) => {
        if (username) {
            ...
            users[socket.id].username = users[socket.id].defaultUsername; // Fallback to default username
        }
...
// Start the server
server.listen(3000, () => {
    console.log('LCS - lantern chat service version 2.6E made by pancakes and learnhelp.cc (BETA) | opened on http://localhost:3000');
});
